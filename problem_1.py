...
class Classifier(nn.Module):
    ...
    """
    def __init__(self, input_size, metric):
...
def train(metric, dist1, dist2, use_cuda=False,setup=3):
    ...
    """
    if setup == 4 :
        D = Classifier(1,metric)
    else:
        D = Classifier(2,metric)
    if use_cuda:
        D.cuda()
    ...
    for it in range(10000):
        ...
        if metric == 'JSD':
            loss = -np.log(2) - torch.mean(torch.log(y_1))/2 - torch.mean(torch.log(1 - y_2))/2
        if metric == 'ce':
...
def estimate(metric, dist1, dist2, D, use_cuda=False):
    ...
    if metric == 'ce':
        return (torch.mean(torch.log(y_1)) + torch.mean(torch.log(1 - y_2))).data.cpu().numpy(),d1,d2
...
if __name__ == "__main__":
    ...
    if args.setup == 4:
        ...
        D = train('ce', dist4, dist3, args.use_cuda,args.setup)
